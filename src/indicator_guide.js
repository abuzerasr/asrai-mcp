/**
 * Indicator interpretation guide — ported from Python indicators_guide.py
...
export function indicator_guide(indicator = "") {
  const key_norm = indicator.toLowerCase();

  if (!indicator || key_norm === "list") {
  ...
  const found =
    Object.keys(GUIDE).find(k => k.toLowerCase() === key_norm) ||
    Object.keys(GUIDE).find(k => k.toLowerCase().startsWith(key_norm)) ||
    Object.keys(GUIDE).find(k => key_norm.includes(k.toLowerCase()));

  if (found) return JSON.stringify({ [found]: GUIDE[found] }, null, 2);

  return JSON.stringify({
    error: `Indicator '${indicator}' not found.`,
    available: Object.keys(GUIDE),
  });
}
